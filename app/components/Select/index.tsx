...
import "./styles.css";
import { Input, useForm } from "@arkyn/components";
...
function Select(props: SelectProps) {
  ...
  const ref = baseRef;
  const isError = props.isError || fieldErrors?.[props.name]?.length > 0;
  ...
  const {
    ...
    onSearch,
    ...rest
  } = getConfig({ ...props, id: props.id, isError }, isFocused);
  ...
  const currentLabel =
    options.find((option: any) => option.value === currentValue)?.label || "";
